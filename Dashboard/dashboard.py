...
df_all = pd.read_csv("https://raw.githubusercontent.com/wildannrr/proyek_analisis_daata/refs/heads/main/Dashboard/airquality_new%20(2).csv")
...
def question_1(df_all):
    with tab1:
        ...
        filtered_data = df_all[df_all['year'] == selected_year]

        # Visualisasi Data
        st.header(f'Perkembangan Rata-rata Polusi Tahun {selected_year}')
    ...
    # visualisasi tren bulanan
        ...
        data_per_year = monthly_avg[monthly_avg['year'] == selected_year]
        ax.plot(data_per_year['month'], data_per_year['Total_Pollutants'], marker='o', label=f'Total Pollutants - {selected_year}')
        ax.set_title(f'Tren Bulanan Total Polutan ({selected_year})')
        ...
        with st.expander("Penjelasan Tabel :"):
            ...
            st.markdown(f"**{descriptions[selected_year]}**")
